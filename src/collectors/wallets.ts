...
import { buildResolvedMarketMap, scoreWallet } from '../analytics/wallet-scorer.js';
...
export async function collectWalletStats(
  ...
  for (const { walletAddress } of toScore) {
    try {
      ...
      scored++;
      if (result.isSharp || result.isProfitable) sharp++;
    } catch (err) {
      childLog.warn({ walletAddress, err }, 'Failed to score wallet — skipping');
    }
  }
